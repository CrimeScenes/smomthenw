...
if getgenv().StartUp.Configuration.FPSUnlock.IsEnabled then
    setfpscap(999)
end

if getgenv().StartUp.Configuration.ConsoleCleaner.Activate then
...
if getgenv().StartUp.Features.Settings.RainbowBars then
...
if getgenv().StartUp.Features.Settings.StretchRes then
    ...
    if getgenv().gg_scripters == nil then
        ...
        game:GetService("RunService").RenderStepped:Connect(function()
            ...
            Camera.CFrame = Camera.CFrame * CFrame.new(0, 0, 0, 1, 0, 0, 0, getgenv().StartUp.Features.Configurations.Resolution.StretchFactor, 0, 0, 0, 1)
        end)
    end
end
...
local function onKeyPress(input, gameProcessed)
    ...
    if input.KeyCode == getgenv().StartUp.Keytrigger.Rejoin and not gameProcessed then
        if getgenv().StartUp.Rejoin.Enabled then  -- Check if the rejoin feature is enabled.
            if getgenv().StartUp.Rejoin.Delay.Use then  -- Check if delay is enabled.
                task.wait(getgenv().StartUp.Rejoin.Delay.Duration)  -- Wait for the specified duration.
            end
            
            -- Rejoin the same server
            print("Rejoining the game...")  -- Indicate action taken.
...
local function performSpin()
    if getgenv().StartUp.Spin.Enabled then  -- Check if spin is enabled
        -- Calculate how many frames to rotate the camera
        for i = 1, math.floor(getgenv().StartUp.Spin.Motion.Degree / getgenv().StartUp.Spin.Motion.Speed) do
            -- Spin the camera by the defined angle each frame
            Camera.CFrame = Camera.CFrame * CFrame.Angles(0, math.rad(getgenv().StartUp.Spin.Motion.Speed), 0)
...
UserInputService.InputBegan:Connect(function(input, gameProcessed)
    if not gameProcessed then  -- Ignore input if it's being processed by a GUI
        if input.KeyCode == getgenv().StartUp.Keytrigger.Spin then  -- Use the key defined in Keytrigger
            performSpin()
        end
    end
end)
...
if getgenv().StartUp.AntiLock.Settings.Enable then
    getgenv().worddot = false
    getgenv().key = tostring(getgenv().StartUp.Keytrigger.AntiLock):lower() -- Get the key from Keytrigger
    getgenv().X = getgenv().StartUp.AntiLock.Velocity.X
    getgenv().Y = getgenv().StartUp.AntiLock.Velocity.Y
    getgenv().Z = getgenv().StartUp.AntiLock.Velocity.Z
...
local function sortInventory()
    if not getgenv().StartUp.InventorySorter.Enabled then return end
    
    local inventory = {}

    -- Collect the player's tools from the Backpack
    for _, item in ipairs(LocalPlayer.Backpack:GetChildren()) do
        local itemName = item.Name:lower()
        if getgenv().StartUp.InventorySorter.Priorities[itemName] then
            table.insert(inventory, item)
        end
    end

    -- Sort items based on defined priorities
    table.sort(inventory, function(a, b)
        return getgenv().StartUp.InventorySorter.Priorities[a.Name:lower()] < getgenv().StartUp.InventorySorter.Priorities[b.Name:lower()]
    end)

    -- Assign tools to specific slots based on priority
    for index, item in ipairs(inventory) do
        local slot = getgenv().StartUp.InventorySorter.Priorities[item.Name:lower()]

        -- Move the item to the character and equip it
        item.Parent = LocalPlayer.Character
        
        -- Optionally, use a method to set the specific slot; this depends on your game's API.
        -- For example, if your game allows setting the tool slot directly:
        if LocalPlayer.Character:FindFirstChild(item.Name) then
            LocalPlayer.Character[item.Name]:SetAttribute("Slot", slot)  -- Replace with the correct method if different
        end
    end
end

-- Keybind for sorting the inventory using the configured key
local keybind = getgenv().StartUp.Keytrigger.InventorySorter -- Use the defined InventorySorter key
...
function Forlorn.sendMouseEvent(x, y, button, isPressed)
    VirtualInputManager:SendMouseButtonEvent(x or 0, y or 0, button or 0, isPressed, game, false)
    UserInputService.MouseIconEnabled = true
end
...
local function getMousePosition()
    return UserInputService:GetMouseLocation().X, UserInputService:GetMouseLocation().Y
end

-- Improved Box FOV Calculation
local function isWithinBoxFOV(position, distance)
    local screenPos = Camera:WorldToViewportPoint(position)
    local screenCenter = Vector2.new(Camera.ViewportSize.X / 2, Camera.ViewportSize.Y / 2)
    
    -- Dynamic FOV scaling based on distance
    local baseFovHeight = getgenv().StartUp.TriggerBot.Settings.BoxFOV.H * 120
    local baseFovWidth = getgenv().StartUp.TriggerBot.Settings.BoxFOV.W * 120
    local scaledFovHeight = baseFovHeight * math.clamp(1 / distance, 0.5, 1.5)
    local scaledFovWidth = baseFovWidth * math.clamp(1 / distance, 0.5, 1.5)

    return (Vector2.new(screenPos.X, screenPos.Y) - screenCenter).Magnitude <= math.sqrt((scaledFovHeight / 2)^2 + (scaledFovWidth / 2)^2)
end
...
local function getPredictedPosition(part)
    if part then
        ...
        local predictionMultiplier = getgenv().StartUp.TriggerBot.Settings.Preds.Mult
        local timeToPredict = getgenv().StartUp.TriggerBot.Settings.Preds.Time or 0.15

        local predictedPosition = part.Position + (velocity * timeToPredict * predictionMultiplier)

        if getgenv().StartUp.TriggerBot.Settings.BoxSync then
            predictedPosition = predictedPosition + Vector3.new(0, part.Size.Y / 2, 0)
        end
...
local function adjustAimForTarget(targetPosition)
    ...
    if onScreen then
        ...
        local offsetX, offsetY = screenPosition.X - centerX, screenPosition.Y - centerY
        
        local smoothFactor = getgenv().StartUp.TriggerBot.Settings.Smoothing or 0.1
        local targetX = centerX + offsetX * smoothFactor
        local targetY = centerY + offsetY * smoothFactor

        -- Adjust mouse position with a small offset for better accuracy
        local adjustmentFactor = 2  -- You can tweak this value for fine-tuning
        targetX = targetX + (offsetX / math.abs(offsetX) * adjustmentFactor)
        targetY = targetY + (offsetY / math.abs(offsetY) * adjustmentFactor)

        VirtualInputManager:SendMouseMoveEvent(targetX, targetY, game)
    end
end

-- Get Closest Part with Improved Logic
local function getClosestPart(player)
    ...
    for _, partName in ipairs(AllBodyParts) do
        ...
        if part and part:IsA("BasePart") and isInLineOfSight(part) then
            ...
            if predictedPosition then
                ...
                -- Adjust for distance when determining the closest part
                if distance < closestDistance and isWithinBoxFOV(predictedPosition, playerDistance) then
                    closestDistance = distance
                    closestPart = predictedPosition
                end
            end
        end
    end
    ...
    for _, player in ipairs(Players:GetPlayers()) do
        if player ~= LocalPlayer and player.Character then
            ...
            if humanoid and humanoid.Health > 0 then
                ...
                if getgenv().StartUp.TriggerBot.Settings.Preds.Safety.IgnoreKnife and 
                   (currentTool and (currentTool.Name:lower() == "knife" or currentTool.Name:lower() == "katana")) then
                    continue
                end

                local targetPart = getClosestPart(player)
                if targetPart and os.clock() - lastClickTime >= getgenv().StartUp.TriggerBot.Settings.CD then
                    lastClickTime = os.clock()
                    
                    -- Adjust aim for the target
                    adjustAimForTarget(targetPart)

                    -- Only fire if the target position is valid
                    if targetPart then
                        local tool = LocalPlayer.Character:FindFirstChildOfClass("Tool")
                        if tool and tool:IsA("Tool") then
                            -- Fire at the predicted position with tool
...
local function handleShootingMode()
    if getgenv().StartUp.TriggerBot.Settings.Mode == "toggle" then
...
LocalPlayer.CharacterAdded:Connect(function(character)
    character:WaitForChild("Humanoid").Jumping:Connect(function(isJumping)
...
UserInputService.InputBegan:Connect(function(input, gameProcessed)
    if not gameProcessed and input.KeyCode == getgenv().StartUp.Keytrigger.TriggerBot then
        handleShootingMode()
    end
end)

UserInputService.InputEnded:Connect(function(input, gameProcessed)
    if input.KeyCode == getgenv().StartUp.Keytrigger.TriggerBot then
        if getgenv().StartUp.TriggerBot.Settings.Mode == "hold" then
...
RunService.RenderStepped:Connect(function()
    if getgenv().StartUp.TriggerBot.Settings.Mode == "hold" and UserInputService:IsKeyDown(getgenv().StartUp.Keytrigger.TriggerBot) then
        TriggerBotAction()
    end
end)
...
Player:GetMouse().KeyDown:Connect(function(Key)
    ...
    if getgenv().StartUp.Macro.Enabled then
        -- Check if the pressed key matches the defined MacroKey
        if Key == tostring(getgenv().StartUp.Keytrigger.Macro):lower() then  -- Use the defined MacroKey
...
local function sendNotification(title, text)
    game.StarterGui:SetCore("SendNotification", {
        Title = title,
        Text = text,
        Duration = getgenv().StartUp.ModCheck.NotificationDuration
    })
end
...
game:GetService("Players").PlayerAdded:Connect(function(player)
    if getgenv().StartUp.ModCheck.Enabled then
        ...
        if specificUserIDs[userId] then
            ...
            if getgenv().StartUp.ModCheck.KickIfModJoined then
...
UserInputService.InputBegan:Connect(function(input, gameProcessed)
    ...
    if getgenv().StartUp.PanicSystem.Settings.IsActive and not gameProcessed then
        -- Check if the pressed key matches the defined ActivationKey from Keytrigger
        if input.KeyCode == getgenv().StartUp.Keytrigger.PanicSystem then
            local customMessage = getgenv().StartUp.PanicSystem.Config.Message
...
if getgenv().StartUp.Configuration.IntroSettings.ShowIntro then
...
if getgenv().StartUp.Memory.Settings.Enable == true then
    local Memory
    ...
    task.spawn(function()
        while task.wait(1) do
            local minMemory = getgenv().StartUp.Memory.Configuration.Start
            local maxMemory = getgenv().StartUp.Memory.Configuration.End
...
local function UpdateFOV()
    ...
    Circle.Visible = StartUp.CamLock.Normal.Radius_Visibility
    Circle.Radius = StartUp.CamLock.Normal.Radius
...
Mouse.KeyDown:Connect(function(Key)
    if Key:lower() == StartUp.CamLock.Normal.Keybind:lower() then
        if StartUp.CamLock.Normal.Enabled then
            if StartUp.CamLock.Normal.mode == "toggle" then
            ...
            elseif StartUp.CamLock.Normal.mode == "hold" then
                IsTargeting = true
                TargetPlayer = ClosestPlrFromMouse()
            end
        end
...
RunService.RenderStepped:Connect(function()
    if IsTargeting and TargetPlayer and TargetPlayer.Character then
        local BodyPart
        if StartUp.CamLock.Normal.ClosestPart then
            BodyPart = GetClosestBodyPart(TargetPlayer.Character)
        else
            BodyPart = TargetPlayer.Character:FindFirstChild(StartUp.CamLock.Normal.HitPart)
        end

        if BodyPart then
            local predictedPosition
            if StartUp['CamLock'].Normal.Resolver then
                local humanoid = TargetPlayer.Character:FindFirstChildOfClass("Humanoid")
                if humanoid then
                    local moveDirection = humanoid.MoveDirection
                    predictedPosition = BodyPart.Position + (moveDirection * StartUp['CamLock'].Normal.Prediction)
                end
            else
                local targetVelocity = TargetPlayer.Character.HumanoidRootPart.Velocity
                predictedPosition = BodyPart.Position + (targetVelocity * StartUp['CamLock'].Normal.Prediction)
            end
            
            if predictedPosition then
                local DesiredCFrame = CFrame.new(Camera.CFrame.Position, predictedPosition)

                if StartUp.CamLock.Normal.SmoothnessEnabled then
                    Camera.CFrame = Camera.CFrame:Lerp(DesiredCFrame, StartUp.CamLock.Normal.Smoothness)
                else
...
-- // Update FOV Function // --
local function updateFov()
    local settings = getgenv().StartUp.Silent.Normal.FovSettings
    clearFovParts()
...
local function isPartInFovAndVisible(part)
    ...
    if not getgenv().StartUp.CamLock.Normal.Enabled or not IsTargeting or not TargetPlayer then
        return false
    end
    ...
    return onScreen and distance <= getgenv().StartUp.Silent.Normal.FovSettings.FovRadius
end


-- // Check if Part Visible // --
local function isPartVisible(part)
    if not getgenv().StartUp.Silent.Normal.WallCheck then 
        return true
    end
...
local OldPredictionY = getgenv().StartUp.Silent.Normal.Prediction
local function GetVelocity(player, part)
    if player and player.Character then
        local velocity = player.Character[part].Velocity
        if velocity.Y < -30 and getgenv().StartUp.Silent.Normal.Resolver then
            getgenv().StartUp.Silent.Normal.Prediction = 0
            return velocity
        elseif velocity.Magnitude > 50 and getgenv().StartUp.Silent.Normal.Resolver then
            return player.Character:FindFirstChild("Humanoid").MoveDirection * 16
        else
            getgenv().StartUp.Silent.Normal.Prediction = OldPredictionY
            return velocity
        end
    end
...
local function toggleFeature()
    getgenv().StartUp.Silent.Normal.Enabled = not getgenv().StartUp.Silent.Normal.Enabled
    local status = getgenv().StartUp.Silent.Normal.Enabled and "StartUp Enabled" or "StartUp Disabled"
    sendNotification("StartUp Notifications", status, "rbxassetid://17561420493")
    if not getgenv().StartUp.Silent.Normal.Enabled then
        Fov.Visible = false
        clearFovParts()
    end
end
...
UserInputService.InputEnded:Connect(function(input, isProcessed)
    if input.KeyCode == Enum.KeyCode[StartUp.CamLock.Normal.Keybind:upper()] and StartUp.CamLock.Normal.mode == "hold" then
...
Run_Service.RenderStepped:Connect(function()
    if getgenv().StartUp.Silent.Normal.Enabled and IsTargeting then  -- Only work when CamLock is engaged
        ...
        if Target and Target.Character then
            ...
            if closestPart and closestPoint then
                ...
                Replicated_Storage[RemoteEvent]:FireServer(Argument, closestPoint + velocity * getgenv().StartUp.Silent.Normal.Prediction)
            end
        end
    else
...
task.spawn(function()
    while task.wait(0.1) do
        if getgenv().StartUp.Silent.Normal.Enabled then
            Target = GetClosestPlr()
            Fov.Visible = IsTargeting and getgenv().StartUp.Silent.Normal.FovSettings.FovVisible  -- Update visibility based on targeting
        end
    end
end)
...
if getgenv().StartUp.Adjustment.Checks.NoGroundShots == true then
    local function CheckNoGroundShots(Plr)
        if getgenv().StartUp.Adjustment.Checks.NoGroundShots and Plr.Character:FindFirstChild("Humanoid") and Plr.Character.Humanoid:GetState() == Enum.HumanoidStateType.Freefall then
            pcall(function()
                local TargetVelv5 = Plr.Character:FindFirstChild(getgenv().StartUp.Silent.Normal and getgenv().StartUp.Silent.Normal)
                if TargetVelv5 then
